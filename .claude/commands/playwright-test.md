...
Before testing, verify the local server is accessible, and identify the running serverPort.

```
mcp__playwright__browser_navigate({ url: "http://localhost:<serverPort>" })
...
```
mcp__playwright__browser_navigate({ url: "http://localhost:<serverPort>/[route]" })
mcp__playwright__browser_snapshot({})
```
